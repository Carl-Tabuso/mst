...
class EmployeeRatingController extends Controller
{
            ...
            ->with([
                ...
                'serviceable.form3'    => function ($query) use ($employee) {
                    $query->with([
                        ...
                        'driver.position',
                        'haulers.position',
                        'teamLeader.performancesAsEmployee' => fn ($q) => $q->where('evaluator_id', $employee->id)
                            ->with('ratings.performanceRating'),
                        'driver.performancesAsEmployee'     => fn ($q) => $q->where('evaluator_id', $employee->id)
                            ->with('ratings.performanceRating'),
                        'haulers.performancesAsEmployee'    => fn ($q) => $q->where('evaluator_id', $employee->id)
                            ->with('ratings.performanceRating'),
                    ]);
                },
            ])
    ...
    public function historyPage(Request $request, $employeeId)
    {
        ...
        $received = $employee->performancesAsEmployee->flatMap(function ($perf) {
            ...
            if (! is_null($scaleFrom) && $scale < $scaleFrom) {
                return false;
            }
            if (! is_null($scaleTo) && $scale > $scaleTo) {
                return false;
            }

            if ($dateFrom) {
                if ($item['job_datetime']->lt(Carbon::parse($dateFrom)->startOfDay())) {
                    return false;
                }
            }
            if ($dateTo) {
                if ($item['job_datetime']->gt(Carbon::parse($dateTo)->endOfDay())) {
                    return false;
                }
            }
