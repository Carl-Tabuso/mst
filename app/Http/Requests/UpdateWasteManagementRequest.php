...
class UpdateWasteManagementRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'appraised_date' => [Rule::requiredIf($isForAppraisal), 'date', 'before_or_equal:'.today()],
            'payment_date'   => [Rule::requiredIf($isSuccessful), 'date', 'before_or_equal:'.today()],
            'bid_bond'       => [Rule::requiredIf($isSuccessful), 'integer'],
            'or_number'      => [Rule::requiredIf($isSuccessful), 'string'],
            'approved_date'  => [Rule::requiredIf($isSuccessful), 'date', 'before_or_equal:'.today()],
            'truck_no'       => ['nullable', 'string'],
            'payment_type'   => [Rule::requiredIf($isSuccessful), 'string'],
            'from'           => [Rule::requiredIf($isPreHauling), 'date', 'after_or_equal:'.today()],
