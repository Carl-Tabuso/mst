...
<template>
  <div class="flex items-center py-1">
    <div class="relative w-full max-w-xs items-center">
      <Input id="search" type="text" class="h-9 max-w-sm pl-12" placeholder="Search" :model-value="globalFilter"
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent>
        ...
        <DropdownMenuCheckboxItem v-for="column in table.getAllColumns().filter((c) => c.getCanHide())" :key="column.id"
          :checked="column.getIsVisible()" @update:checked="(value: boolean) => column.toggleVisibility(value)">
      ...
      <Button :disabled="!hasRowSelection" :variant="hasRowSelection ? 'destructive' : 'secondary'" class="mx-3"
        ...
        <template v-if="hasRowSelection">
          <div class="hidden lg:flex">
            <Badge variant="destructive" class="rounded-full font-normal">{{ table.getSelectedRowModel().rows.length }}
            </Badge>
          </div>
        </template>
      </Button>
