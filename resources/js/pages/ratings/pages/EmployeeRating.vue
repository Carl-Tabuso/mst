...
function getRole(order: any, coworker: Coworker): { role: string, department: string } {
  ...
  if (form3.team_leader?.id === coworker.id) return { role: '(Team Leader)', department: 'Hauling Department' };
  if (form3.driver?.id === coworker.id) return { role: '(Driver)', department: 'Hauling Department' };
  if (form3.safetyOfficer?.id === coworker.id) return { role: '(Safety Officer)', department: 'Hauling Department' };
  if (form3.mechanic?.id === coworker.id) return { role: '(Mechanic)', department: 'Hauling Department' };
  if ((form3.haulers || []).some((h: any) => h.id === coworker.id)) return { role: '(Hauler)', department: 'Hauling Department' };
...
function allRated(order: any): boolean {
  ...
  return getCoworkers(order).every(coworker =>
    currentRatings.some(r => r.evaluatee_id === coworker.id && r.performance_rating_id)
  );
}
    ...
    <div v-for="order in props.jobOrders" :key="order.id" class="p-12 px-32">
      ...
      <div class="mb-8 w-full">
        <div class="flex justify-between items-start mb-1">
          ...
          <div class="text-xl text-right font-semibold text-blue-900">
            Job Order: #{{ order.id }}
          </div>
        </div>
      ...
      <div class="px-12">
        <div v-if="getCoworkers(order).length" class="divide-y divide-gray-100">
          <CoworkerRatingForm v-for="coworker in getCoworkers(order)" :key="coworker.id"
            ...
            :rating="ratings[order.serviceable?.form3?.id]?.find(r => r.evaluatee_id === coworker.id)?.performance_rating_id || 0"
            :description="ratings[order.serviceable?.form3?.id]?.find(r => r.evaluatee_id === coworker.id)?.description || ''"
            @update:rating="n => setRating(order.serviceable?.form3?.id, coworker.id, n)"
        ...
        <div v-else class="p-8 text-center text-gray-500 italic">
          No co-workers to rate for this hauling. It means you were the only one assigned to this job order HAHA.
        </div>
      </div>
      ...
      <div v-if="getCoworkers(order).length" class="mt-6">
        ...
        <textarea v-model="overallRemarks"
          ...
          rows="4" placeholder="Enter your overall remarks here..." />
      </div>

      <!-- Action Buttons -->
      <div v-if="getCoworkers(order).length" class="flex justify-end gap-3 mt-6">
        <button type="button"
