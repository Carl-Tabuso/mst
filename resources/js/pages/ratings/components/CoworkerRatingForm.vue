    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex items-center gap-3">
          ...
          <div class="flex gap-1">
            <button v-for="n in 5" :key="n" @click="$emit('update:rating', n)"
              ...
              :class="n <= Number(rating) ? 'text-lime-500 dark:text-lime-400' : 'text-gray-300 dark:text-gray-600'">
              ★
            </button>
          </div>

          <span class="min-w-[2rem] text-sm font-medium text-gray-700 dark:text-gray-300">
            <span class="text-lime-500 dark:text-lime-400">{{ rating || 0 }}</span>/5
          </span>
        </div>
        ...
        <textarea :value="description" @input="
          ...
          " class="w-full resize-none border-0 border-b 
         ...
         dark:placeholder-gray-500 dark:focus:border-blue-400" placeholder="Add feedback..." rows="1" />
      </div>
    </div>
  </div>
</template>
