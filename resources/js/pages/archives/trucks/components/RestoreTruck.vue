...
interface RestoreTruckProps {
  truck: Truck
}

const props = defineProps<RestoreTruckProps>()

const isRestoreModalOpen = ref<boolean>(false)
const isSubmitting = ref<boolean>(false)

const onRestore = () => {
  router.patch(route('archive.truck.update', props.truck.id), undefined, {
...
const canRestoreArchivedTruck = usePermissions().can('restore:truck')
</script>
    ...
    <Tooltip>
      <TooltipTrigger as-child>
        <div>
          <DialogTrigger
            as-child
            :disabled="!canRestoreArchivedTruck"
          >
            <Button
              type="icon"
      ...
      <TooltipContent>
        {{ canRestoreArchivedTruck ? 'Restore' : 'Unauthorized' }}
      </TooltipContent>
    </Tooltip>
    <DialogContent class="w-[350px]">
      <VisuallyHidden as-child>
        <DialogTitle> Restoring Truck </DialogTitle>
        <DialogDescription>
          <!---->
        </DialogDescription>
      </VisuallyHidden>
      <div class="flex flex-col items-center justify-center gap-2">
        ...
        <div class="text-3xl font-bold">Restoring Truck</div>
        <div class="space-y-2 text-center text-sm text-muted-foreground">
          <p>Are you sure you want to restore this truck?</p>
          <div class="mt-1">
            <p>
