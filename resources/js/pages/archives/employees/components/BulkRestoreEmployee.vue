...
interface BulkRestoreEmployeeProps {
  table: Table<Employee>
}

const props = defineProps<BulkRestoreEmployeeProps>()
...
const onBulkRestore = () => {
  router.patch(
    route('archive.employee.bulk_restore'),
    {
  ...
  <Dialog>
    ...
    <DialogContent class="w-fit">
      <VisuallyHidden as-child>
        <DialogTitle>Restoring Job Employees</DialogTitle>
        <DialogDescription>
          <!---->
        </DialogDescription>
      </VisuallyHidden>
      <div class="flex flex-col items-center justify-center gap-2">
        ...
        <div class="text-3xl font-bold">
          Restoring {{ selectedRowModels.length }} Employee(s)
        </div>
