...
function goTo(page: number) {
    router.get(route('archives.index'), { page }, { preserveState: true, preserveScroll: true })
}
...
<template>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 px-4">
        ...
        <div class="flex items-center gap-2">
            <button class="px-3 py-1 border rounded text-sm hover:bg-gray-100 disabled:opacity-50"
                :disabled="props.pagination.current_page <= 1" @click="goTo(props.pagination.current_page - 1)">
                Previous
            </button>

            <template v-for="page in paginationPages" :key="page">
                <span v-if="page === '...'" class="px-2 text-gray-400">…</span>
                <button v-else class="px-3 py-1 rounded text-sm font-medium" :class="{
                    'bg-blue-600 text-white': page === props.pagination.current_page,
                    'text-gray-700 hover:bg-gray-100': page !== props.pagination.current_page
                }" @click="goTo(page as number)" :disabled="page === props.pagination.current_page">
                    {{ page }}
                </button>
            </template>
