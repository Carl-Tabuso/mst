...
export const useITServiceForm = (props: ITServiceFormProps) => {
  ...
  const hasDuplicate = computed(() => {
    return form.machine_infos.some(machine =>
      props.existingServices.some(service =>
  ...
  const isValidMachine = computed(() =>
    form.machine_infos.every(machine =>
  ...
  return { form, machineFieldsEnabled, hasDuplicate, isValidMachine, submitForm, addMachineInfo };
};
