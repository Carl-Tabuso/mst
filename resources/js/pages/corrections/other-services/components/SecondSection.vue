      ...
      <div class="flex w-full flex-col">
        <div class="grid gap-y-6">
          ...
          <div class="grid grid-cols-2 gap-x-10 gap-y-3">
            <div class="flex items-center gap-x-4">
              ...
              <div class="w-full">
                ...
                />
                <p
                  v-if="serviceableChangedFields.includes('assigned_person')"
                  class="mt-1 text-xs text-muted-foreground"
                >
                  Changed from:
                  {{
                    getAssignedPersonDisplay(serviceableBefore.assigned_person)
                  }}
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center">
              ...
              <div class="w-full">
                ...
                />
                <p
                  v-if="serviceableChangedFields.includes('purpose')"
                  class="mt-1 text-xs text-muted-foreground"
                >
          ...
          <div
            v-if="
              itemsChanged &&
              serviceableBefore.items &&
              serviceableBefore.items.length > 0
            "
            class="grid grid-cols-[auto,1fr] gap-x-7 gap-y-3 rounded-md border border-red-200 bg-red-50 p-3"
          >
            ...
            <div class="space-y-3">
              <div class="space-y-2">
                <div
                  ...
                  :key="index"
                  class="flex items-center gap-3 rounded-md border border-red-200 p-2"
                >
                  <Input
                    :model-value="item.item_name || 'Unnamed item'"
                    class="pointer-events-none flex-1 border-red-200 bg-red-100"
                  />
                  <Input
                    :model-value="`Qty: ${item.quantity}`"
                    class="pointer-events-none w-20 border-red-200 bg-red-100"
                  />
                </div>
              </div>
            </div>
